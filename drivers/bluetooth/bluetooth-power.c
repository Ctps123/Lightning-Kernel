	...
	if (!vreg->is_enabled) {
		if (vreg->set_voltage_sup) {
			rc = regulator_set_voltage(vreg->reg,
						...
						vreg->high_vol_level);
			if (rc < 0) {
				BT_PWR_DGB("vreg_set_vol(%s) failed rc=%d\n",
						vreg->name, rc);
				goto out;
			}
		}
		...
		if (vreg->load_uA >= 0) {
			rc = regulator_set_load(vreg->reg,
					vreg->load_uA);
			if (rc < 0) {
				BT_PWR_DGB("vreg_set_mode(%s) failed rc=%d\n",
						vreg->name, rc);
				goto out;
			}
		}
		...
		if (rc < 0) {
			BT_PWR_DGB("regulator_enable(%s) failed. rc=%d\n",
					vreg->name, rc);
			goto out;
		}
...
static int bt_vreg_disable(struct bt_power_vreg_data *vreg)
{
	...
	if (vreg->is_enabled) {
		...
		if (rc < 0) {
			BT_PWR_DGB("regulator_disable(%s) failed. rc=%d\n",
					vreg->name, rc);
			goto out;
		}
...
static int bt_clk_enable(struct bt_power_clk_data *clk)
{
	...
	if (!clk->clk || clk->is_enabled) {
		BT_PWR_DGB("error - node: %p, clk->is_enabled:%d",
	...
	if (rc) {
		BT_PWR_DGB("failed to enable %s, rc(%d)\n", clk->name, rc);
		return rc;
	}
...
static int bt_configure_gpios_2wcn(int on)
{
	...
	if (on) {
		...
		if (rc) {
			BT_PWR_DGB("unable to request gpio %d (%d)\n",
					bt_3p3_en_gpio, rc);
			return rc;
		}
		...
		if (rc) {
			BT_PWR_DGB("Unable to set direction\n");
			return rc;
		}
		msleep(50);
...
static int bluetooth_power(int on)
{
	...
	if (on) {
		...
		if (rc < 0) {
			BT_PWR_DGB("bt_power regulators config failed");
			goto regulator_fail;
		}
		...
		if (bt_power_pdata->bt_chip_clk) {
			...
			if (rc < 0) {
				BT_PWR_DGB("bt_power gpio config failed");
				goto clk_fail;
			}
		}
			...
			char *buf)
{
	...
	if (ret) {
		BT_PWR_DGB("Platform capability info from CNSS not available!");
		enable = false;
...
static int bluetooth_power_rfkill_probe(struct platform_device *pdev)
{
	...
	if (ret < 0)
		BT_PWR_DGB("device create file error!");
		...
		struct bt_power_clk_data **clk_data)
{
	...
	*clk_data = NULL;
	if (of_parse_phandle(np, "clocks", 0)) {
		...
		if (!clk) {
			BT_PWR_DGB("No memory for clocks");
			ret = -ENOMEM;
			goto err;
		}
		...
		ret = of_property_read_string_index(np, "clock-names", 0,
				&(clk->name));
		if (ret < 0) {
			BT_PWR_DGB("reading \"clock-names\" failed");
			return ret;
		}
		...
		if (IS_ERR(clk->clk)) {
			ret = PTR_ERR(clk->clk);
			BT_PWR_DGB("failed to get %s, ret (%d)",
				clk->name, ret);
			clk->clk = NULL;
			return ret;
		}
		...
		*clk_data = clk;
		...
		BT_PWR_DGB("clocks is not provided in device tree");
	}
	...
	if (pdev->dev.of_node) {
		...
		bt_power_pdata->bt_gpio_sys_rst =
			of_get_named_gpio(pdev->dev.of_node,
						"qca,bt-reset-gpio", 0);
		if (bt_power_pdata->bt_gpio_sys_rst < 0)
			BT_PWR_DGB("bt-reset-gpio not provided in device tree");
		...
		if (rc < 0)
			BT_PWR_DGB("clock not provided in device tree");
	}
...
static int bt_power_probe(struct platform_device *pdev)
{
	...
	if (!bt_power_pdata) {
		BT_PWR_DGB("Failed to allocate memory");
		return -ENOMEM;
	}
	...
	if (pdev->dev.of_node) {
		...
		if (ret < 0) {
			BT_PWR_DGB("Failed to populate device tree info");
			goto free_pdata;
		}
		...
		BT_PWR_DGB("Failed to get platform data");
		goto free_pdata;
	}
...
static long bt_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case BT_CMD_CHIPSET_VERS:
		...
		BT_PWR_DGB("BT_CMD_CHIP_VERS soc_version:%x", chipset_version);
		if (chipset_version) {
		soc_id = chipset_version;
		} else {
			BT_PWR_DGB("got invalid soc version");
			soc_id = 0;
		}
		break;
	default:
...
static int __init bluetooth_power_init(void)
{
	...
	if (bt_major < 0) {
		BT_PWR_DGB("failed to allocate char dev\n");
		goto chrdev_unreg;
	}

	bt_class = class_create(THIS_MODULE, "bt-dev");
	if (IS_ERR(bt_class)) {
		BT_PWR_DGB("coudn't create class");
		goto chrdev_unreg;
	}
